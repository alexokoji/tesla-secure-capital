...
-- ---------- Withdrawal verification codes (admin-issued) --------------------
-- User requests -> admin generates -> delivered via notifications -> consumed
-- at withdrawal. Codes never expire; they're invalidated only when used.
...
-- User files a request (no code yet); deduped if one is already open.
CREATE OR REPLACE FUNCTION public.request_withdrawal_code()
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF EXISTS (SELECT 1 FROM public.withdrawal_codes
             WHERE user_id = auth.uid() AND status IN ('requested','issued')) THEN
    RETURN;
  END IF;
  INSERT INTO public.withdrawal_codes (user_id, status) VALUES (auth.uid(), 'requested');
END;
$$;
GRANT EXECUTE ON FUNCTION public.request_withdrawal_code() TO authenticated;

-- Admin generates + delivers the code; returns it so the admin can relay it too.
...
BEGIN
  ...
  IF v_id IS NULL THEN
    INSERT INTO public.withdrawal_codes (user_id, code, status, issued_at)
      VALUES (p_user_id, c, 'issued', now()) RETURNING id INTO v_id;
  ELSE
    UPDATE public.withdrawal_codes SET code = c, status = 'issued', issued_at = now() WHERE id = v_id;
  END IF;
  UPDATE public.withdrawal_codes SET status = 'used', used_at = now()
    WHERE user_id = p_user_id AND status = 'issued' AND id <> v_id;
  INSERT INTO public.notifications (user_id, title, body)
    VALUES (p_user_id, 'Your withdrawal code',
...
-- Validate + consume the caller's issued code (no expiry).
...
BEGIN
  ...
  SELECT id INTO v_id FROM public.withdrawal_codes
    WHERE user_id = auth.uid() AND code = p_code AND status = 'issued'
...
-- Admin listing of requests (joins profile; rows returned only to admins).
